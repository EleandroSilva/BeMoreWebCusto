...
Type
  ...
  private
    ...
    //procedure LoopTabelaPreco(aValue);
  public
    constructor Create;
...
begin
  ...
  begin
    ...
    BuscaQtdeProducao(qrSelect.FieldByName('IdModelo').AsString);
...
begin
  ...
  FAliquotaMarkup := dmPrincipal.dsAuxiliar.DataSet.FieldByName('SomaAliquota').AsCurrency;
  FIndiceaVistaMultiplicar := dmPrincipal.dsAuxiliar.DataSet.FieldByName('IndiceMultiplicacao').AsCurrency;
  FIndiceaVistaDividir := dmPrincipal.dsAuxiliar.DataSet.FieldByName('IndiceDivisao').AsCurrency;

  lSoma := aCustoFinal;
  lSoma := lSoma * FIndiceaVistaMultiplicar;

  qrUpdate.Active := False;
  qrUpdate.SQL.Clear;
  qrUpdate.SQL.Text := ('update MontarCusto mc set ' +
    'ValorVendaaVista= ' + StringReplace(CurrToStr(lSoma), ',', '.', [RFReplaceAll]) + ' ' +
    'where mc.id=' + aValue + ' ');
  qrUpdate.ExecSQL;
end;
...
begin
  ...
  qrUpdate.SQL.Clear;
  qrUpdate.SQL.Text := ('update MontarCusto mc set ' +
    'QtdeProducao='+IntToStr(FQtdeProducao)+','+
  ...
  BuscarMarkup(aValue,qrSelect.FieldByName('CustoTotalModelo').AsCurrency);
