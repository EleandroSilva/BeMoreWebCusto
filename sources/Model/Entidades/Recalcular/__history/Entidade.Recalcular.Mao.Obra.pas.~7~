unit Entidade.Recalcular.Mao.Obra;
...
var
  lValorFrete, lValorMaoObra, lValorAjustado, lValorCusto: Currency;
begin
  lValorFrete := 0;
  lValorMaoObra := 0;
  lValorAjustado := 0;
  ...
  qrAuxiliar.SQL.Create.Text := ('select ' +
    ...
    'coalesce(cast(sum(pitem.valorAjustado) as numeric (10,2)),0) ValorAjustado ' +
  ...
  lValorAjustado := qrAuxiliar.FieldByName('ValorAjustado').AsCurrency;
  lValorCusto := StrToFloat(FormatFloat('0.00', lValorAjustado + lValorFrete));
  try
    qrAuxiliar.Active := False;
    qrAuxiliar.SQL.Clear;
    qrAuxiliar.SQL.Text := ('update Producao p set ' +
      ...
      'p.ValorAjustado=' + StringReplace(CurrToStr(lValorAjustado), ',', '.', [RFReplaceAll]) + ', ' +
      'p.ValorCusto=' + StringReplace(CurrToStr(lValorCusto), ',', '.', [RFReplaceAll]) + ' ' +
      'where p.ID=' + FId + ' ');
  finally
    qrAuxiliar.ExecSQL;
  end;
