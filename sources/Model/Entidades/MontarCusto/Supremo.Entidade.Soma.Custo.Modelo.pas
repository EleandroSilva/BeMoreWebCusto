...
function TSomaCustoModelo.Aliquota1(aValue: Currency): iModelEntidade;
begin
  Result := Self;
  FAliquota1 := aValue;
end;

function TSomaCustoModelo.Aliquota2(aValue : Currency) : iModelEntidade;
begin
  Result := self;
  FAliquota2 := aValue;
end;

function TSomaCustoModelo.Aliquota3(aValue : Currency) : iModelEntidade;
begin
  Result := self;
  FAliquota3 := aValue;
end;

function TSomaCustoModelo.Aliquota4(aValue : Currency) : iModelEntidade;
begin
  Result := self;
  FAliquota4 := aValue;
end;
...
const
  cSQL = ('select ' +
    ...
    ' p.valorfrete, '+
    ' p.valorajustado, '+
    ' coalesce(cast(sum(fti.valorfinal) as numeric(10,3)),0) ValorFichaTecnica ' +
    ', ' +
    '( ' +
    ' select ' +
    '    coalesce(cast(sum(df.valor) as numeric(10,3)),0) ValorDespesaFixa ' +
    '   from despesafixa df ' +
    ' ) ' +
    ' , ' +
    '( ' +
    ' select ' +
    '    coalesce(cast(sum(df.valor/ft.qtdeproducao) as numeric(10,3)),0) CustoModeloDespesaFixa ' +
    '   from despesafixa df ' +
    ' ) ' +
    'from fichatecnica ft ' +
    'inner join Modelo             m on m.Id               = ft.idmodelo ' +
    'inner join fichatecnicaitem fti on fti.idfichatecnica = ft.id '+
    'left  join producao           p on p.idmodelo         = ft.idmodelo ');
var
