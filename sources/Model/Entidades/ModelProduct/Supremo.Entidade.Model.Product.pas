...
const
 cSQL = (
         ...
         'select '+
         'count(*) QtdeModelo '+
         'from ModeloProdutoItem mpi1 '+
         'where mpi1.idmodelo = mpi.idmodelo '+
         ') '+
         ...
         'inner join ModeloProdutoItem mpi on mpi.id         = mp.id ' +
         'inner join Modelo              m on m.id           = mpi.idmodelo '+
...
begin
  ...
  if FTextoFiltro <> '' then
    case FEscolhaFiltro of
      ...
      1: lWhere := lWhere + ('and mpi.IdModelo=' + aValue + ' ');
      2: lWhere := lWhere + ('and m.Descricao CONTAINING ''' + aValue + ''' ');
      3: lWhere := lWhere + ('and mp.Operador CONTAINING ''' + aValue + ''' ');
    end;

  lOrdem :=('order by mpi.IdModelo asc, (pro.descricao||'' ''||cor.descricao) ');
