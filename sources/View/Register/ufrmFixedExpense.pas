...
procedure TfrmFixedExpense.btnPrintClick(Sender: TObject);
begin
  if not dmPrincipal.cdsDespesaFixa.IsEmpty then
    dmEntidadeRelatorios.pprDespesaFixa.Print
    else
...
begin
  ...
  if aValue <> '' then
    case rgChoice.ItemIndex of
      0: lWhere := ('where df.id=' + aValue + ' ');
      1: lWhere := ('where df.IdClasseContas=' + aValue + ' ');
      2: lWhere := ('Where ccc.Descricao CONTAINING ''' + aValue + ''' ');
      3: lWhere := ('Where df.Operador CONTAINING ''' + aValue + ''' ');
    end;

  lOrdem := ('Order by df.IdClasseContas asc, ccc.Descricao ');
