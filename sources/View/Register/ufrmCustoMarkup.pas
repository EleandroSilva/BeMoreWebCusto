  ...
  case pgMain.ActivePageIndex of
    ...
    4:
      if not dmPrincipal.cdsAliquotaGrupo.IsEmpty then
        if MessageBox(handle, 'Deseja excluir registro completo, caso quiser excluir apenas a linha clique em não!', 'Aviso',
  ...
  case pgMain.ActivePageIndex of
    2:
      if not PesquisaMarkupNome(edtSearchMarkupNome.Text) then
        ShowMessage('Registro não encontrado, favor tentar outro filtro!');
    3:
...
begin
  ...
  if TryStrToCurr(TextOnEdit, Value) then
    ...
    edtAliquota.Text := '0,00'; // se caso não conseguiu converter o valor digitado em número, mostra 0
...
begin
  ...
  begin
    ...
    raise Exception.Create('Atenção... ' + edtOperator.Text + ' Registro não encontrado. Verifique ' + #13 +
      'a tabela Nome de Alíquota, se consta um cadastro com, ' + #13 +
      'Alíquota de Financeiro > que 0, Estando com a opção de ' + #13 +
      'Financeiro marcada como 1 - Sim. Caso não encontrar, ' +
      'Cadastre apenas uma opção na tabela Nome de Alíquota. ' + #13 +
      'Opção encontrada na tela Conjunto Custo ' + #13 +
      'Markup na segunda aba Nome Alíquota no canto ' + #13 +
      'direito da tela um quadrado com duas opções Financeiro 0 - Não e 1 - Sim. Marque a Opção 1 - Sim e faz o cadastro');
  end;
end;
...
begin
  ...
  begin
    ...
    raise Exception.Create('Atenção... ' + edtOperator.Text + ' Registro não encontrado, tente outro filtro!');
  end;
end;
