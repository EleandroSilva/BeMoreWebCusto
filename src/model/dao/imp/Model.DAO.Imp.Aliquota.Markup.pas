...
begin
  ...
  FSQL :=('select '+
        ...
        '  ValorLucroReal, '+
        '  case '+
        '   when status=1 then (ValorMarkupComLucro-Valor) '+
        '   when status=0 then 0 '+
        '   end valormarkupsemlucro ' +
        '  from '+
        '     ( '+
        '      select '+
        '        mt.id, '+
        ...
        '        ag.aliquota, '+
        '        coalesce(cast(sum(ag.aliquota*mt.valorvendaavista/100) as numeric(10,4)),0) as Valor, ');
  lSQL :=(' ( '+
        '    select '+
        '      coalesce(cast(sum(ag1.aliquota*mt.valorvendaavista/100) as numeric(10,4)),0) as valormarkupcomlucro '+
  ...
  lSQL1 :=(' ( '+
        '       select '+
        '         coalesce(cast(sum(ag2.aliquota*mt.valorvendaavista/100)+mt.valorfichatecnica as numeric(10,4)),0) as ValorFichatecnicaMarkup '+
  ...
  lSQL2   :=('(select '+
             'coalesce(cast(sum(ag3.aliquota*mt.valorvendaavista/100) as numeric(10,4)),0) as ValorLucroReal '+
  ...
  lResult :=(FSQL+lSQL+lWhere+lSQL1+lWhere1+lSQL2+lWhere2+lSQL3+' where mt.Id='+IntToStr(Id)+ ' group by 1, 2, 3, 4, 5, 6, 7)');
  try
    try
      FDataSet := FQuery
                    .SQL(lResult)
